...
class GetModelMixin(BaseModelRouteMixin):
    get_model: ClassVar[type[BaseModel] | None] = None
    get_fields: ClassVar[type[Enum] | None] = None
    ...
    def __bootstrap__(cls, **kwargs) -> None:
        ...
        parameters["fields"] = parameters["fields"].replace(
            annotation=Optional[list_anno1],  # noqa: UP007,UP045
        )
    ...
    async def get(
        ...
        fields: list[TModelFields] | None = Query(None),
        # *args,
        # **kwargs,
    ) -> PartialModelResponse:
        result: BaseModel | None = await self.repository.get(id_, fields)
        return PartialModelResponse(result)


class ListModelMixin(BaseModelRouteMixin):
    __filtering__: Any | None = None
    ...
    async def list(
        ...
        next_: int | str | None = Query(None, alias="next"),
...
class CreateModelMixin(BaseModelRouteMixin):
    create_model: ClassVar[type[BaseModel] | None] = None
    ...
    def __bootstrap__(cls, *args, **kwargs) -> None:
        ...
        parameters = signature.parameters.copy()
        if (
            parameters["model"].annotation == "BaseModel | TEntity"
    ...
    async def create(self, model: BaseModel | TEntity) -> TEntity:
...
class ReplaceModelMixin(BaseModelRouteMixin):
    ...
    replace_model: ClassVar[type[BaseModel] | None] = None
    ...
    async def replace(self, model: BaseModel | TEntity, id_: Any = Path(alias="id")) -> Any:
...
class PatchModelMixin(BaseModelRouteMixin):
    ...
    patch_model: ClassVar[type[BaseModel] | None] = None
    ...
    async def patch(self, model: BaseModel | TEntity, id_: Any = Path(alias="id")) -> Any:
...
class DeleteModelMixin(BaseModelRouteMixin):
    ...
    async def delete(self, id_: str | int = Path(..., alias="id")) -> None:
...
class BulkCreateModelMixin(BulkBase):
    ...
    def __bootstrap__(cls, *args, **kwargs) -> None:
        ...
        parameters = signature.parameters.copy()
        if (
            parameters["bulk"].annotation == "list[BaseModel | TEntity]"
    ...
    async def bulk_create(self, bulk: list[BaseModel | TEntity]) -> BulkResponseModel:
        return await self.repository.bulk_create(bulk)


class BulkUpdateModelMixin(BulkBase):
    update_model: ClassVar[type[BaseModel] | None] = None
